		...
		public function print_script() {
			...
			if ( 'enabled' != self::$file_generation || self::$fallback_js ) {
				if ( !is_null( self::$script ) || ( '' != self::$script ) ) {
					ob_start();
					?>
					<script type="text/javascript" id="ive-script-frontend"><?php echo self::$script; //phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?></script>
					<?php
					ob_end_flush();
				}
			}
			...
			if ( class_exists( 'WooCommerce' ) ) {
		        ...
		        if ( isset( $ibtana_ecommerce_product_addons_license_key['license_key'] ) ) {
		          if ( ( $ibtana_ecommerce_product_addons_license_key['license_key'] != '' ) ) {
					...
					if( ( ! wp_style_is( $style_woocommerce_premium_frontend, 'queue' ) ) && ( ! wp_style_is( $style_woocommerce_premium_frontend, 'done' ) ) && $is_iepa_active ) {
						wp_enqueue_style(
